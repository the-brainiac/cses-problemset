...
def generate_difference_arr(arr):
    diff = [0]*(n)
    diff[0] = arr[0]
    for i in range(1,n):
...
# for _ in range(readInt()):
    ...
    n, q = intMap()
    arr = intList()
    diff = generate_difference_arr(arr)
    seg = generate_segment(diff)
    n = 2**math.ceil(math.log2(len(diff)))
    for q0 in range(q):
        t, *args = intList()
        if t ==1:
            a, b, u = args
            update(a-1, seg[n+a-1]+u)
            try:
                update(b, seg[n+b]-u)
            except:
                pass
        else:
            print(query(0, args[0]-1))
